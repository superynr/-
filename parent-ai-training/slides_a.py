...
def s02(prs):
    ...
    notes(s, "네 가지를 읽어 주고 손을 들게 합니다. 가장 많이 나온 항목을 기억해 두었다가 "
             "해당 구간(숙제=21~25장, 사실 확인=28~29장, 시작 나이=9~15장)에서 다시 언급하면 몰입도가 올라갑니다.\n"
...
def s04(prs):
    ...
    notes(s, "정답은 X입니다. 투표 결과를 먼저 세어 보고 시작하면 다음 장의 연구 결과가 훨씬 잘 들립니다.\n"
             "여기서 결론을 길게 설명하지 마십시오. 5~8장에서 근거를 제시합니다. "
...
def s05(prs):
    ...
    notes(s, "비유: ‘엄청나게 많은 글을 읽고 다음에 올 말을 아주 잘 맞히는 프로그램’. "
             ...
             "여기서 ‘환각(hallucination)’이라는 용어를 한 번만 언급하고 넘어가되, 28장에서 다시 다룬다고 예고합니다. "
...
def s06(prs):
    ...
    notes(s, "A와 B는 같은 AI, 같은 문제입니다. 다른 것은 아이가 무엇을 요청했는가뿐입니다. "
             "24장 ‘답 알려줘를 바꿔봅시다’ 활동이 바로 이 갈림길을 B로 돌리는 연습이라고 연결해 주십시오.")
    return s
...
def s09(prs):
    ...
    notes(s, "학부모가 가장 많이 묻는 질문이지만, 하나의 숫자로 답하지 않는 것이 정확합니다.\n"
             "다음 장에서 세 번째 관문(준비도)을, 13장에서 첫 번째 관문(서비스별 연령 조건)을 구체적으로 다룹니다. "
             "두 번째 관문(학교 지침)은 11~12장, 21장입니다.")
    return s
