...
def s22(prs):
    ...
    footer(s, 23, SRC_KPF, link=URL_KPF)
...
def s23(prs):
    ...
    y = title(s, "AI가 출처를 달면 믿어도 될까요?", kicker="사실 확인",
              sub="출처까지 달려 나오면 더 믿음이 갑니다. 그런데 그게 진짜인지는 또 다른 문제입니다.")
    ...
    rich(s, ML + 0.4, y + 3.82, CW - 0.8, 0.7,
         [[("중·고등학생의 ", "body", False), ("39%가 AI 답변을 신뢰", "teal", True),
          ...
          [("초등학생은 이 조사의 대상이 아닙니다. 다만 확인하는 습관은 초등 시기에 자리 잡습니다.",
            "body", False)]],
         size=14.5, spacing=1.35, space_after=4)
    footer(s, 24, SRC_NYPI, link=URL_NYPI)
...
def s24(prs):
    ...
    rich(s, ML + 0.4, y + 3.46, CW - 0.8, 0.7,
         ...
         size=14.5, spacing=1.35, space_after=4)
    footer(s, 25, "이 답변은 활동을 위해 강사가 만든 가상 예시입니다. 실제 조사 결과가 아닙니다.")
...
def s25(prs):
    ...
    rich(s, ML + 0.4, y + 3.7, CW - 0.8, 0.9,
         ...
         size=14.5, spacing=1.35, space_after=4)
    footer(s, 26, SRC_MOE, link=URL_MOE)
...
def s27(prs):
    ...
    footer(s, 28, "참고: OpenAI 자녀 보호 기능 안내 (신고·차단·보호자 설정)", link=URL_OPENAI_PC)
