...
def s21(prs):
    ...
    notes(s, "이 프롬프트는 유인물에 그대로 넣어 드리는 것이 좋습니다. 부모가 아이 옆에서 한 번만 같이 써 보면 "
             "아이가 형식을 금방 익힙니다.\n"
             "네 번째(답변 방식)가 핵심입니다. {bastani}장 연구에서 효과를 만든 것이 바로 이 부분, "
             "즉 ‘정답 대신 힌트’ 설정이었습니다.\n"
             "주의: 좋은 공식이 답을 유용하게 만들어 주지만 정확성까지 보장하지는 않습니다. {factcheck}장에서 이어집니다.")
    return s
...
def s23(prs):
    ...
    notes(s, "‘AI는 거짓말을 한다’보다 ‘AI는 모르면 그럴듯하게 채운다’가 정확한 표현입니다({how}장과 연결). "
...
def s26(prs):
    ...
    txt(s, bx + 0.32, y + 2.55, bw - 0.64, 0.3, "층과 상관없이", size=12,
        bold=True, color="teal")
    txt(s, bx + 0.32, y + 2.88, bw - 0.64, 0.8,
        ["출처 표시와 개인정보 관리는", "모든 층에 걸칩니다.", "다음 장에서 정리했습니다."],
    ...
    notes(s, "‘우리 아이만 뒤처지는 것 아닌가’와 ‘몇 학년에 뭘 가르쳐야 하나’에 한꺼번에 답하는 장입니다.\n"
             ...
             "그게 곧 {formula}장에서 배운 프롬프트 공식입니다.\n"
...
def s27(prs):
    ...
    notes(s, "실제로 가장 많이 어긋나는 지점입니다. 놀란 부모가 먼저 화를 내면 아이는 다음부터 숨깁니다.\n"
             ...
             "경우도 있습니다({account}장). 다만 기능은 자주 바뀌므로 각자 확인이 필요하다고 안내합니다.\n"
