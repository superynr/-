...
def s21(prs):
    ...
    y = title(s, "좋은 질문은 네 가지를 담습니다", kicker="질문의 구조",
              sub="이 네 가지가 들어가면 AI의 답이 ‘아이에게 맞는 답’이 됩니다.")
    items = [("내 수준", "“초등학교 4학년이야”"), ("내 시도", "“분모끼리 더했어”"),
             ("필요한 도움", "“어디가 틀렸는지 알고 싶어”"), ("답변 방식", "“정답 말고 질문으로 물어봐 줘”")]
    cw = (CW - 3 * 0.3) / 4
...
def s22(prs):
    ...
    rich(s, ML + 0.4, y + 3.16, CW - 0.8, 0.75,
         [[("매일 쓰는 아이는 아직 많지 않습니다. ", "body", False),
           ("초등학생 중 ‘매일 이용’은 3.7%", "teal", True),
           ("입니다.", "body", False)],
          [("지금은 금지할 때가 아니라, 습관이 자리 잡기 전에 사용 방식을 정해 둘 시기입니다.",
            "body", False)]],
...
def s23(prs):
    s = blank(prs)
    y = title(s, "AI가 출처를 달면 믿어도 될까요?", kicker="사실 확인",
              sub="출처가 붙어 있다는 것과 출처가 맞다는 것은 다릅니다.")
    steps = [("링크가 실제로 열리나?", "존재하지 않는 주소를\n만들어 내기도 합니다."),
             ("원문에 그 내용이 있나?", "링크는 진짜인데 내용은\n다르게 요약되기도 합니다."),
             ("날짜와 대상이 맞나?", "오래된 자료이거나\n다른 나라 이야기일 수 있습니다.")]
    cw, gap = 3.6, 0.42
    for i, (h, b) in enumerate(steps):
        x = ML + i * (cw + gap)
        rrect(s, x, y + 0.28, cw, 2.05, fill="tint", line="line", radius=0.1)
        numbadge(s, x + 0.32, y + 0.52, 0.42, i + 1, fill="blue")
        txt(s, x + 0.32, y + 1.08, cw - 0.64, 0.4, h, size=16.5, bold=True, color="ink", spacing=1.15)
        txt(s, x + 0.32, y + 1.52, cw - 0.64, 0.7, b.split("\n"), size=13, color="body", spacing=1.3)
    rrect(s, ML, y + 2.6, CW, 0.82, fill="redbg", line=None, radius=0.1)
    txt(s, ML + 0.4, y + 2.82, CW - 0.8, 0.4,
        "같은 AI에게 “정말 맞아?”라고 다시 묻는 것은 독립적인 검증이 아닙니다.",
        size=15.5, bold=True, color="red")
    rrect(s, ML, y + 3.62, CW, 1.08, fill="tintteal", line=None, radius=0.1)
    rich(s, ML + 0.4, y + 3.82, CW - 0.8, 0.7,
         [[("청소년의 ", "body", False), ("39%가 AI 답변을 신뢰", "teal", True),
           ("한다고 답했지만, 온라인 정보의 진위가 불분명할 때 출처를 확인하는 등 ", "body", False),
           ("능동적으로 검증한다는 응답은 21.2%", "teal", True), ("에 그쳤습니다.", "body", False)]],
         size=14.5, spacing=1.35)
    footer(s, 23, SRC_NYPI)
    notes(s, "‘AI는 거짓말을 한다’보다 ‘AI는 모르면 그럴듯하게 채운다’가 정확한 표현입니다(5장과 연결).\n"
             "수치 안내: 한국청소년정책연구원 조사에서 ‘생성형 AI가 제공하는 정보가 사실인지 확인할 수 있다’는 "
             "자기 평가는 3.36점, ‘정보가 특정 견해에 치우쳐 있는지 판단할 수 있다’는 3.45점으로 낮게 나타났습니다. "
             "학업 성취도가 낮을수록 허위 정보를 구분하기 어려워하는 경향도 보고되었습니다.\n"
             "주의: 특정 모델의 오류율을 모든 AI의 특성처럼 말하지 않도록 합니다. 서비스와 시점에 따라 다릅니다.")
    return s
...
def s24(prs):
    ...
    rich(s, ML + 0.4, y + 3.46, CW - 0.8, 0.7,
         [[("집에서 쓰는 한 문장: ", "body", False),
          ...
          [("아이를 시험하는 말이 아니라, 함께 확인하자는 초대가 되도록 말해 주세요.", "body", False)]],
         size=14.5, spacing=1.35, space_after=4)
    footer(s, 24)
...
def s25(prs):
    ...
    rich(s, ML + 0.4, y + 3.7, CW - 0.8, 0.9,
         [[("학교 평가에서도 개인정보 보호는 별도 영역으로 다뤄집니다.", "teal", True)],
          [("과제에 AI를 활용할 때 무엇을 입력했는지까지 살피도록 안내되어 있습니다. "
            "집에서도 같은 기준을 쓰면 아이가 헷갈리지 않습니다.", "body", False)]],
         size=14.5, spacing=1.35, space_after=4)
    footer(s, 25, SRC_MOE)
...
def s26(prs):
    s = blank(prs)
    y = title(s, "어떤 도구를 미리 배워두면 좋을까요?", kicker="준비시킬 것",
              sub="특정 AI 제품의 조작법보다, 어디서나 쓰이는 다섯 가지가 먼저입니다.")
    ...
    rich(s, ML + 0.4, y + 3.16, CW - 0.8, 0.72,
         [[("AI 제품은 계속 바뀝니다. ", "body", False),
           ("바뀌지 않는 것은 위의 다섯 가지입니다.", "teal", True)],
          [("AI는 이 활동을 하다가 막힐 때 도움을 얻는 방법으로 익히면 충분합니다.", "body", False)]],
         size=14.5, spacing=1.35, space_after=4)
    footer(s, 26)
    notes(s, "‘우리 아이만 뒤처지는 것 아닌가’라는 불안에 답하는 장입니다.\n"
...
def s27(prs):
    ...
    rich(s, ML + 0.4, y + 3.0, CW - 0.8, 0.95,
         [[("혼내면 다음부터 말하지 않습니다.", "red", True)],
          [("아이가 이상한 답변을 보여 주는 것은 문제가 생겼다는 신호가 아니라, "
            "부모를 믿고 있다는 신호입니다.", "body", False)],
    ...
    notes(s, "실제로 가장 많이 어긋나는 지점입니다. 놀란 부모가 먼저 화를 내면 아이는 다음부터 숨깁니다.\n"
             ...
             "경우도 있습니다(13장). 다만 기능은 자주 바뀌므로 각자 확인이 필요하다고 안내합니다.\n"
...
def s30(prs):
    ...
    txt(s, ML, 4.85, CW, 0.4,
        "아이의 생각을 지키는 일은 AI를 막는 데서 시작하지 않습니다.",
        size=16, color="body", align=PP_ALIGN.CENTER)
    txt(s, ML, 5.3, CW, 0.4,
        "먼저 묻고, 함께 확인하고, 자기 말로 설명하게 하는 데서 시작합니다.",
        size=16, color="body", align=PP_ALIGN.CENTER)
