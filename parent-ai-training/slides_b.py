...
def s14(prs):
    ...
    footer(s, 16)
    notes(s, "신호등은 냉장고에 붙여 두기 좋은 형태입니다. {promise}장에서 각 가정이 직접 채웁니다.\n"
...
def s17(prs):
    ...
    footer(s, 19, SRC_MOE, link=URL_MOE)
    notes(s, "학부모가 가장 궁금해하는 지점입니다. 핵심은 ‘무조건 안 된다’가 아니라 ‘교사가 정한 범위 안에서’입니다.\n"
             ...
             "우리 학교·우리 학년의 실제 기준은 {checklist}장의 체크리스트로 확인하는 것이 정확합니다.\n"
...
def s18(prs):
    ...
    footer(s, 20, SRC_PNAS, link=URL_PNAS)
    notes(s, "{bastani}장의 연구와 직접 연결됩니다. 정답을 그대로 주는 방식(4단계 쪽)이 학습을 해쳤고, "
...
def s14a(prs):
    """Family Link에서 제미나이 켜는 방법"""
    s = blank(prs)
    y = title(s, "Family Link에서 제미나이 켜는 방법", kicker="사용 설정",
              sub="허용하기로 정하셨다면, 부모 휴대폰에서 이 순서로 설정합니다.")
    steps = [("Family Link 앱 열기", "부모 휴대폰에서\n앱을 실행합니다"),
             ("자녀 프로필 선택", "제미나이를 쓸\n아이를 고릅니다"),
             ("설정(Controls)", "메뉴에서 설정\n항목을 엽니다"),
             ("제미나이(Gemini)", "목록에서 해당\n메뉴를 찾습니다"),
             ("스위치 켜기", "Gemini 앱을\n‘켬(On)’으로")]
    ...
    for i, (h, b) in enumerate(steps):
        x = ML + i * (cw + gap)
        last = (i == 4)
        rrect(s, x, y + 0.35, cw, 1.95, fill="tintblue" if last else "tint",
              line=None if last else "line", radius=0.1)
        numbadge(s, x + cw / 2 - 0.21, y + 0.6, 0.42, i + 1, fill="blue" if last else "teal")
        txt(s, x + 0.1, y + 1.2, cw - 0.2, 0.5, h, size=13.5, bold=True, color="ink",
            align=PP_ALIGN.CENTER, spacing=1.15)
        txt(s, x + 0.1, y + 1.72, cw - 0.2, 0.5, b.split("\n"), size=11.5, color="body",
            align=PP_ALIGN.CENTER, spacing=1.25)
        if not last:
            arrow(s, x + cw + gap / 2 - 0.16, y + 1.22, 0.32, 0.26, fill="line")

    rrect(s, ML, y + 2.55, CW, 1.12, fill="tintteal", line=None, radius=0.1)
    txt(s, ML + 0.4, y + 2.75, 4.5, 0.3, "켜기 전에 아이와 함께 정하세요", size=13.5,
        bold=True, color="teal")
    items = ["무엇을 알아볼 때 쓸지", "어떤 정보는 넣지 않을지", "언제 끝낼지"]
    bw = (CW - 0.8) / 3
    for i, it in enumerate(items):
        txt(s, ML + 0.4 + i * bw, y + 3.15, bw - 0.2, 0.35, "· " + it, size=14,
            color="ink")

    rrect(s, ML, y + 3.85, CW, 0.8, fill="tint", line="line", radius=0.1)
    txt(s, ML + 0.4, y + 4.03, CW - 0.8, 0.5,
        ["메뉴 이름은 앱 버전에 따라 조금씩 다를 수 있습니다. 설정이 반영되는 데 몇 시간 걸리고,",
         "계정에 따라 아직 제미나이가 지원되지 않을 수도 있습니다."],
        size=12.5, color="mute", spacing=1.3)
    footer(s, 15, "출처: Google For Families 고객센터 「자녀의 Gemini 앱 사용 관리하기」",
           link=URL_GEMINI)
    notes(s, "{compare}장에서 Gemini가 △였던 이유가 이 설정입니다. 실제로 해 보실 분들을 위한 화면입니다.\n"
             "진행 요령: 다섯 단계를 천천히 읽어 주시고, 관심 있는 분은 지금 휴대폰으로 따라 해 보시라고 "
             "해도 좋습니다. 다만 강사가 먼저 한 번 해 보고 오시는 편이 좋습니다. "
             "메뉴 이름과 위치가 앱 버전에 따라 다를 수 있습니다.\n"
             "구글이 안내하는 주의사항 두 가지를 꼭 함께 전해 주세요. ①감독 대상 계정에 대한 제미나이 "
             "지원이 점진적으로 확대되는 중이라 아직 보이지 않을 수 있습니다. ②설정을 바꿔도 반영되는 데 "
             "몇 시간이 걸릴 수 있고, 안 되면 앱을 다시 시작해 보라고 안내되어 있습니다.\n"
             "가장 중요한 것은 아래 띠입니다. 켜는 방법보다 켜기 전에 정하는 세 가지가 오늘 연수의 핵심입니다. "
             "‘설정은 5분이면 끝나지만, 약속은 계속 쓰입니다’라고 정리해 주시면 좋겠습니다.\n"
             "이 장은 제미나이를 권하는 자리가 아닙니다. 허용하기로 이미 정한 가정을 위한 안내라는 점을 "
             "한 번 짚어 주십시오. {promise}장 ‘우리 집 AI 사용 약속’에서 이 세 가지를 그대로 씁니다.")
    return s
